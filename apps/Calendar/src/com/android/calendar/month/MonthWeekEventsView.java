...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    protected void loadColors(Context context) {
        ...
        mFestivalColor = res.getColor(R.color.festival_color);
    ...
    protected void initView() {
        ...
        mFestivalPaint.setTextSize(TEXT_SIZE_MONTH_NUMBER);
        mFestivalPaint.setStyle(Style.FILL);
        mFestivalPaint.setTextAlign(Align.RIGHT);
        mFestivalPaint.setTypeface(Typeface.DEFAULT);
        if (DEBUG_LAYOUT) {
        ...
        for (; i < numCount; i++) {
            ...
            if (LunarUtils.showLunar() && service != null) {
                ...
                try {
                    ...
                    String mFestival = service.getFestival(year,month, monthDay);
                    String TraditionalFestival = service.getTraditionalFestival(year,month, monthDay);
                    String chunjie = getMyString(R.string.chunjie);
                    String yuanxiao = getMyString(R.string.yuanxiao);
                    String qingming = getMyString(R.string.qingming);
                    String duanwu = getMyString(R.string.duanwu);
                    String qixi = getMyString(R.string.qixi);
                    String zhongqiu = getMyString(R.string.zhongqiu);
                    String chongyang = getMyString(R.string.chongyang);
                    String laba = getMyString(R.string.laba);
                    String xiaonian = getMyString(R.string.xiaonian);
                    String chuxi = getMyString(R.string.chuxi);
                    if (!TextUtils.isEmpty(display)) {
                        float originalTextSize = mMonthNumPaint.getTextSize();
                        float originalTextSize1 = mFestivalPaint.getTextSize();
                        boolean isTraditional =false;
                        Resources res = getResources();
                        TEXT_SIZE_LUNAR = res.getInteger(R.integer.text_size_lunar);
                        mMonthNumPaint.setTextSize(TEXT_SIZE_LUNAR);
                        int mOrientation = res.getConfiguration().orientation;
                        mFestivalPaint.setTextSize(TEXT_SIZE_LUNAR);
                        if (display.equals(chunjie)||
                            display.equals(yuanxiao)||
                            display.equals(qingming)||
                        	display.equals(duanwu)||
                        	display.equals(qixi)||
                        	display.equals(zhongqiu)||
                        	display.equals(chongyang)||
                        	display.equals(laba)||
                        	display.equals(xiaonian)||
                        	display.equals(chuxi))
                        {
                        	isTraditional=true;
                        }
                        if (display.indexOf(chunjie) >= 0 ||
                        display.indexOf(yuanxiao) >= 0||
                        display.indexOf(qingming) >= 0||
                    	display.indexOf(duanwu)>= 0||
                    	display.indexOf(qixi)>= 0||
                    	display.indexOf(zhongqiu)>= 0||
                    	display.indexOf(chongyang)>= 0||
                    	display.indexOf(laba)>= 0||
                    	display.indexOf(xiaonian)>= 0||
                    	display.indexOf(chuxi)>= 0)
                    {
                    	isTraditional=true;
                    }

                        if (!TextUtils.isEmpty(mFestival) || (isTraditional)) {
                        	Log.d(TAG, "display ==="  + display);
                        	String display1 = "";
                        	if (display.length() > 3) {
                        		display1 = display.substring(0,display.indexOf('\n'));
                        		display = display.substring(display.lastIndexOf('\n'));
                        		canvas.drawText(display1, x, y + mMonthNumHeight + TOP_PADDING_MONTH_NUMBER-LAND_Y_REDUCTION , mFestivalPaint);
                        		canvas.drawText(display, x, y + mMonthNumHeight + TOP_PADDING_MONTH_NUMBER-LAND_Y_REDUCTION + 30, mMonthNumPaint);  
                            	
							}else {
								canvas.drawText(display, x, y + mMonthNumHeight + TOP_PADDING_MONTH_NUMBER-LAND_Y_REDUCTION, mFestivalPaint);  
							}
                        	
						} else {
							 canvas.drawText(display, x, y + mMonthNumHeight + TOP_PADDING_MONTH_NUMBER-LAND_Y_REDUCTION, mMonthNumPaint);
						}
                        // restore the text size.
                        mMonthNumPaint.setTextSize(originalTextSize);
                        mFestivalPaint.setTextSize(originalTextSize1);
                    }
