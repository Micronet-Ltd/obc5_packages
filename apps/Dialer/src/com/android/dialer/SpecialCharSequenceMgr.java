...
/*added by xuegang for factory test 20150817 begin*/
private static boolean isArrayContains(Object[] array, Object obj) {
...
private static void startFactoryTestActivity(Context context, boolean isAttchment) {
	...
	try {
		...
		Log.i(TAG, "factorykit not found");
	}
}
...
private static boolean handleSetPropDisplay(Context context, String input) {
	Intent mIntent = new Intent(Intent.ACTION_MAIN);
	mIntent.setClassName("com.vendor.setprop", "com.vendor.setprop.SetPropActivity");//modfiy  by jiayugang add getversion time function (general) 20140711
	mIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
	mIntent.putExtra("inputString", input);
	try {
		context.startActivity(mIntent);
	} catch (ActivityNotFoundException e) {
	   Log.i(TAG, "SetPropActivity not found");
	}
	return true;
}
