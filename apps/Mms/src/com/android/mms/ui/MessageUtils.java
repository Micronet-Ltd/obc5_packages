...
import android.graphics.BitmapFactory;
import java.util.concurrent.ExecutorService;   //lihui@20151020 added for resolve the exception when select a lot of image as mms attachment.
import java.util.concurrent.Executors;         //lihui@20151020 added for resolve the exception when select a lot of image as mms attachment.
...
public class MessageUtils {
    ...
    private static final int POOL_SIZE = 2;        //lihui@20151020 added for resolve the exception when select a lot of image as mms attachment.
    private static ExecutorService mExecutorPool = Executors.newFixedThreadPool(POOL_SIZE);  //lihui@20151020 added for resolve the exception when select a lot of image as mms attachment.
    ...
    public static void resizeImageAsync(final Context context,
		...
		/*lihui@20151020 modified for resolve the exception when select a lot of image as mms attachment start*/
        //new Thread(new Runnable() {
        mExecutorPool.execute(new Runnable() {
        /*lihui@20151020 modified for resolve the exception when select a lot of image as mms attachment end*/
            @Override
            public void run() {
                final PduPart part;
                try {
