...
bool client_Access_Allowed = true;
...
bool open()
{
    ...
    client_Access_Allowed = stat?true:false;

    ALOGD("%s: Exit. stat = 0x%02x  client_Access_Allowed = 0x%02x", fn,stat,client_Access_Allowed);
    return stat;
}
/*******************************************************************************
**
...
bool close()
{
    ...
    {
        ALOGD("%s: Client Access Denied..Returning..", fn);
        return false;
    }
    if(eSE_connected != true)
        return true;
