...
static jbyteArray nativeNfcSecureElement_doTransceive (JNIEnv* e, jobject, jint handle, jbyteArray data)
{
    ...
    if (NFCSTATUS_SUCCESS == EXTNS_NfcReqResourceIoctl(gGeneralTransceiveTimeout * 1000))
    {
        ...
        EXTNS_NfcReleaseResourceIoctl(gGeneralTransceiveTimeout * 1000);
        ALOGD("%s: exit: recv len=%ld", __FUNCTION__, recvBufferActualSize);
        return result;
    }
    else
    {
        ALOGD("%s: exit: recv len=0", __FUNCTION__);
        return NULL;
    }
}
