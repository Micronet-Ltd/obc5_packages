...
public class FileManagerService extends Service {
    ...
    public static final int FILE_FILTER_TYPE_VCARD = 4;
    public static final int FILE_FILTER_TYPE_AUDIO = 5;
    ...
    public boolean isBusy(String activityName) {
        ...
        if (task != null
            ...
            LogUtils.w(TAG, "Task has been started.");
            return true;
        }
        return false;
    }
    ...
    public void pasteFiles(String activityName, List<FileInfo> fileInfoList,
        ...
        if (filterPasteList(fileInfoList, dstFolder) > 0) {
            listener
                    .onTaskResult(OperationEventListener.ERROR_CODE_PASTE_TO_SUB);
        }
        FileInfoManager fileInfoManager = getActivityInfo(activityName)
                .getFileInfoManager();
        if (fileInfoManager == null) {
        ...
        if (fileInfoList.size() > 0) {
            ...
            case FileInfoManager.PASTE_MODE_CUT:
                if (isCutSamePath(fileInfoList, dstFolder)) {
                    listener
                            .onTaskResult(OperationEventListener.ERROR_CODE_CUT_SAME_PATH);
                    return;
                }
    ...
    public void listFiles(String activityName, String path,
        ...
        if (isBusy(activityName)) {
            ...
            if (fileInfoManager != null) {
                BaseAsyncTask task = new ListFileTask(fileInfoManager,
                        listener, path, filterType);
                getActivityInfo(activityName).setTask(task);
                task.execute();
            }
        }
     ...
     */
    /*QRT Begin modified by yanlei 20131216  do not search the hidden file when set don't show hidden file*/
    public void search(String activityName, String searchName, String path,
            OperationEventListener operationEvent,boolean isShowHidenFile) {
        if (isBusy(activityName)) {
            cancel(activityName);
        ...
        if (fileInfoManager != null) {
            BaseAsyncTask task = new SearchTask(fileInfoManager,
                    operationEvent, searchName, path, getContentResolver(),isShowHidenFile);
            getActivityInfo(activityName).setTask(task);
            task.execute();
        }
    }
    /*QRT End modified by yanlei 20131216  do not search the hidden file when set don't show hidden file*/
