...
public final class FileUtils {
    ...
    public static final String SEP_STR = File.separator;
        ...
        if (TextUtils.isEmpty(fileName) || fileName.trim().length() == 0) {
            ...
            try {
                ...
                if (length > FileInfo.FILENAME_MAX_LENGTH) {
                    LogUtils.d(TAG, "fileName Length = " + length);
    ...
    public static String getMultipleMimeType(FileManagerService service, String currentDirPath,
        ...
        for (FileInfo info : files) {
            ...
            mimeType = mMimeTypes.getMimeType(info.getFile().getName());
            if ((null != mimeType)
    ...
    public static boolean isHidden(String path) {
        ...
        if (path != null && path.length() > 0) {
            if (path.startsWith(DOT_STR) || path.indexOf(SEP_STR + DOT_STR) >= 0) {
                return true;
            }
        }
    ...
    public static String getBaseName(String path) {
        if (path != null) {
            int dot = path.lastIndexOf(DOT_STR);
            if (dot >= 0) {
