...
public class WebCorpus extends MultiSourceCorpus {
    ...
    public WebCorpus(Context context, Config config, SearchSettings settings, Executor executor,
        ...
        /*modified by liulijun for SW00034551 to block search from internet 14-2-25 begin */
        //super(context, config, executor, webSearchSource, browserSource);
        super(context, config, executor, browserSource);
        /*modified by liulijun for SW00034551 to block search from internet 14-2-28 end */
        if (DBG) {
    ...
    protected class WebResult extends Result {
        ...
        public void fill() {
            ...
            if (browserResult != null && browserResult.getCount() > 0) {
                /*modified by liulijun for SW00034551 add search result to show 14-2-25 begin */
                int count = browserResult.getCount();
                for(int i=0; i<count;i++){
                    add(new SuggestionPosition(browserResult, i));
                }
                /*modified by liulijun for SW00034551 add search result to show 14-2-28 end */
            }
