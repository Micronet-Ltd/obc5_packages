...
public class SearchActivityViewSinglePane extends SearchActivityView {
    ...
    public void onResume() {
         /*modified by zhangjiaquan for SW00029976 Set google corpus when current corpus is deleted 14-1-15 begin */
         List<Corpus> listOfEnabledCorpora = getQsbApplication().mCorpora.getEnabledCorpora();
	  Corpus currentCorpus = getCorpus();
	  if(!listOfEnabledCorpora.contains(currentCorpus))
	  {
	      setCorpus(null,null);
	  }
         /*modified by zhangjiaquan for SW00029976 Set google corpus when current corpus is deleted 14-1-15 end */
   ...
   /*added by zhangjiaquan for SW00029976 Set google corpus when current corpus is deleted 14-1-15 begin */
    protected void setCorpus(Corpus corpus,String corpusName) {
        super.setCorpus(corpusName);
        if (mCorpusIndicator != null) {
            Drawable sourceIcon;
            if (corpus == null) {
                sourceIcon = getContext().getResources().getDrawable(R.mipmap.search_app_icon);
    ...
    /*added by zhangjiaquan for SW00029976 Set google corpus when current corpus is deleted 14-1-15 end */
