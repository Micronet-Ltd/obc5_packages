...
public class ReadWriteOperations {
    ...
    public static final String TAG = "ReadWriteOperations";
    ...
    public static void writeConfigurationToFile(Boolean override, Context context) {
        ...
        if (!file.exists()) {
            Log.d(TAG, "CellularDataServiceConfig.txt not found, write the default setting: " + handlerValue);
        }
        try {
            FileOutputStream fileOutputStream = new FileOutputStream(file);
            fileOutputStream.write(handlerValue.getBytes());
    ...
    public static String readConfigurationFromFile(Context context) {
        ...
        Log.d(TAG, "Read Override Config: " + ret);
        return ret;
    }
    ...
    public static void LogToFile(String disabledCount, String enabledCount, Context context, String additionalMessage, ContentResolver contentResolver) throws IOException {
        ...
        String serviceActivityHeader = "Time Stamp, Message Information, Core Temperatures, Override Cell State Configuration, isMobileDataEnabled, isAirplaneModeEnabled, isWiFi Enabled,  Enabled Count, Disabled Count,";
        ...
        String[] tempValues = TemperatureValues.temperaturevalues.split(":");
        String coreTemps = tempValues[1];
        String delim = ",";
        ...
        try {
            ...
            bufferedWriter.write(String.valueOf(MobileDataManager.isWifiConnected(context)) + delim);
    ...
    public static void serviceActivityLog(Boolean pauseStatus, String activity, String reason, Context context) {

        String header = "TimeStamp, Action Information, Trigger/Reason, Service Running, Core Temperatures, isMobileDataEnabled, isAirplaneMode Enabled, isWiFi Enabled";
        TemperatureValues.getThermalZoneTemp();
        String[] tempValues = TemperatureValues.temperaturevalues.split(":");
        String coreTemps = tempValues[1];
        String delim = ",";
        ...
        String timestamp = Utils.formatDate(System.currentTimeMillis()); //Getting current time stamp
        File file = new File(serviceDir, "ServiceActivityLog.csv");//Created a Text File to maintain the service activity log

        if (!file.exists()) {
            isNewFile = true;
            Log.d(TAG, "ServiceActivityLog.csv: File Doesn't exist");
        }
        try {
            fileWriter = new FileWriter(file.getAbsoluteFile(), true);
            bufferedWriter = new BufferedWriter(fileWriter);
            ...
            bufferedWriter.write(timestamp + delim);
            bufferedWriter.write(activity + delim);
            bufferedWriter.write(reason + delim);
            bufferedWriter.write(String.valueOf(pauseStatus) + delim);
            bufferedWriter.write(coreTemps + delim);
            bufferedWriter.write(String.valueOf(MobileDataManager.getMobileDataState(context) + delim));
            bufferedWriter.write(String.valueOf(MobileDataManager.isAirplaneMode(context.getContentResolver())) + delim);
            bufferedWriter.write(String.valueOf(MobileDataManager.isWifiConnected(context)) + delim);
            bufferedWriter.newLine();
        } catch (IOException e) {
            Log.e("Exception", "File write failed: " + e.toString());
