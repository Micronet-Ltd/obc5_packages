...
//del by liulijun to delete this import because there is no android.provider.Applications in 4.4 (8916) HQ00000000 2014.06.30
...
public class ApplicationsProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        ...
        /*modified by liulijun for SW00035008 update datatable before query 14-3-5 begin */
        //postUpdateAll();
        updateApplicationsList(null);
        /*modified by liulijun for SW00035008 update datatable before query 14-3-5 end */
    ...
    private Cursor searchApplications(String query, String[] projectionIn,
        ...
        String orderBy = null;
        if (!zeroQuery) {
                 //add by liulijun to avoid single quotation in string (general) HQ00000000 2014-7-4
                 query = query.replace("'", "''");
        	 qb.appendWhere(buildTokenFilter(query) + " OR " + NAME + " LIKE '%" + query + "%'"); /*modify by liulijun for SW00045777 2014.05.05*/
        } else {
            if (hasGlobalSearchPermission()) {
    ...
    private String buildTokenFilter(String filterParam) {
        ...
        /*modified by zhangjiaquan for SW00024274 search by key word 2013-12-14 begin */
        DatabaseUtils.appendEscapedSQLString(filter,
                "*" + DatabaseUtils.getHexCollationKey(filterParam) + "*");
        /*modified by zhangjiaquan for SW00024274 search by key word 2013-12-14 end */
