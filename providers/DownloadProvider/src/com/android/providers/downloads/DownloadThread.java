...
public class DownloadThread implements Runnable {
    ...
    private void parseOkHeaders(HttpURLConnection conn) throws StopRequestException {
		...
		if(mInfo.mHint != null && mInfo.mHint.length()>=4){
			String hint = mInfo.mHint.substring((mInfo.mHint.length()-4),mInfo.mHint.length()).toLowerCase();
			if(((mInfoDelta.mMimeType.toLowerCase()).equals("application/octet-stream"))&&(hint.equals(".apk"))){
				mInfoDelta.mMimeType="application/vnd.android.package-archive";
			}
		}
